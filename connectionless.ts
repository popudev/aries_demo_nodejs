...
const initializeHolderAgent = async () => {
  const genesisTransactionsBCovrinTestNet = await getGenesisTransaction("http://localhost:9000/genesis");
...
const sendProofRequest = async (issuer: Agent, credentialDefinitionId: any, connectionId: any) => {
  ...
  await issuer.proofs.requestProof(connectionId, {
    name: "proof-request",
    nonce: "1298236324864",
...
const acceptProofRequest = async (holder: Agent, proofRecordId: string) => {
  const retrievedCredentials = await holder.proofs.getRequestedCredentialsForProofRequest(proofRecordId, {
    filterByPresentationPreview: true,
  });

  //hard code
  const credentials = retrievedCredentials.requestedAttributes.name;
  ...
  await holder.proofs.acceptRequest(proofRecordId, requestedCredentials);
};
  ...
  issuer.events.on<ProofStateChangedEvent>(ProofEventTypes.ProofStateChanged, async ({ payload }) => {
    switch (payload.proofRecord.state) {
      ...
      case ProofState.PresentationReceived:
        ...
        const presentationAttachments = payload.proofRecord.presentationMessage?.presentationAttachments;
...
const PrensentProofConnectionless = async (issuer: Agent, holder: Agent, credentialDefinitionId: string) => {
  ...
  const { requestMessage } = await issuer.proofs.createOutOfBandRequest({
    name: "proof-request",
    nonce: "1298236324864",
    requestedAttributes: proofAttribute,
  ...
  await holder.receiveMessage(requestMessage.toJSON());
};
